...
function PbbContent() {
  ...
  const handleInquiry = async () => {
    if (nop.length < 6) {
      setError("Nomor Objek Pajak minimal 6 digit");
      return;
    }
    setLoading(true);
    setError(null);
    try {
      ...
      if (!data.inquiry.targetValid || !data.inquiry.inquiryId) {
        setError(data.inquiry.errorMessage || "Nomor Objek Pajak tidak valid");
        setLoading(false);
        return;
      }
      setInquiry(data);
  ...
  return (
    <div className="min-h-dvh flex flex-col bg-white">
      {step !== "result" && step !== "processing" && (
        <ServiceHeader title={headerTitle[step] || "PBB"} onBack={handleBack} />
      )}
      ...
      {step === "input" && (
        <div className="flex-1 px-5 pt-6">
          <IdNumberInput
            ...
            value={nop}
            onChange={(v) => { setNop(v); setError(null); }}
            onSubmit={handleInquiry}
            error={error}
            icon={<Building2 size={18} className="text-[var(--color-text-secondary)]" />}
          />
          <button
            onClick={handleInquiry}
            disabled={loading || nop.length < 6}
            className={cn(
              "w-full h-12 rounded-xl font-semibold text-white mt-6 transition-colors flex items-center justify-center gap-2",
              loading || nop.length < 6
      ...
      {step === "result" && (
        <TransactionResult
          ...
          receiptRows={
            result
              ? [
                  { label: "NOP", value: result.target.number },
