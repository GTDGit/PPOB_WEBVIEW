...
function VoucherGameContent() {
  ...
  const handleInquiry = async () => {
    if (gameId.length < 3) {
      setError("Game ID minimal 3 karakter");
      return;
    }
    setLoading(true);
    setError(null);
    try {
      ...
      if (!data.inquiry.targetValid || !data.inquiry.inquiryId) {
        setError(data.inquiry.errorMessage || "Game ID tidak valid");
        setLoading(false);
        return;
      }
  ...
  const headerTitle: Record<string, string> = {
    provider: "Voucher Game",
    input: provider?.name || "Voucher Game",
    products: "Pilih Voucher",
    confirm: "Konfirmasi",
    pin: "Konfirmasi",
  };
  ...
  return (
    <div className="min-h-dvh flex flex-col bg-white">
      {step !== "result" && step !== "processing" && (
        <ServiceHeader title={headerTitle[step] || "Voucher Game"} onBack={handleBack} />
      )}
      ...
      {step === "input" && (
        <div className="flex-1 px-5 pt-6">
          ...
          <IdNumberInput
            ...
            value={gameId}
            onChange={(v) => { setGameId(v); setError(null); }}
            onSubmit={handleInquiry}
            error={error}
            icon={<Gamepad2 size={18} className="text-[var(--color-text-secondary)]" />}
          />
          <button
            onClick={handleInquiry}
            disabled={loading || gameId.length < 3}
            className={cn(
              "w-full h-12 rounded-xl font-semibold text-white mt-6 transition-colors flex items-center justify-center gap-2",
              loading || gameId.length < 3
      ...
      {step === "products" && (
        <div className="flex-1 px-5 pt-4 pb-4 overflow-y-auto">
          <div className="flex items-center gap-3 mb-4 p-3 rounded-xl bg-[var(--color-accent-soft)]">
            ...
            <div>
              <p className="text-sm font-medium text-[var(--color-text-primary)]">{gameId}</p>
      ...
      {step === "confirm" && orderData && (
        <ConfirmSheet pricing={orderData.pricing} payment={orderData.payment} loading={loading} onConfirm={handleConfirmPay}>
          <div className="p-4 rounded-xl bg-[var(--color-accent-soft)] mb-4">
            ...
            <p className="text-sm text-[var(--color-text-secondary)] mt-1">Game ID: {orderData.target.number}</p>
          </div>
        </ConfirmSheet>
      )}
      ...
      {step === "result" && (
        <TransactionResult
          ...
          receiptRows={
            result
              ? [
                  { label: "Game", value: provider?.name || "" },
                  { label: "Voucher", value: result.product.name },
                  { label: "Game ID", value: result.target.number },
